#!/usr/bin/env bash
# Download the Gateway API CRDs for a pinned kubernetes-sigs/gateway-api
# release tag and synthesise swagger-style definitions for the catalogued
# gateway kinds (src/k8s/kinds.go) into swagger/crd/gateway-api.json — the
# second input to cmd/gentypes (merged with the core swagger). The core
# swagger never carries Gateway API kinds (they are CRDs), so without this
# file they fall back to the permissive body type.
#
# Usage:
#   GATEWAY_API_VERSION=v1.6.2 scripts/update-gateway-api.sh
#
# The kind/version pairs below must match the gateway.networking.k8s.io
...
: "${GATEWAY_API_VERSION:?set GATEWAY_API_VERSION (a kubernetes-sigs/gateway-api release tag, e.g. v1.6.2 — never main)}"
if ! [[ "$GATEWAY_API_VERSION" =~ ^v[0-9]+\.[0-9]+(\.[0-9]+)?$ ]]; then
  echo "ERROR: GATEWAY_API_VERSION must be a release tag like v1.6.2 (got '$GATEWAY_API_VERSION')" >&2
  exit 1
fi
...
base="https://raw.githubusercontent.com/kubernetes-sigs/gateway-api/$GATEWAY_API_VERSION/config/crd/standard"
target="$root/swagger/crd/gateway-api.json"
tmpdir=$(mktemp -d)
trap 'rm -rf "$tmpdir"' EXIT

# CRD file -> (Kind, version) pairs, matching src/k8s/kinds.go.
files=("gateways:Gateway:v1" "httproutes:HTTPRoute:v1" "referencegrants:ReferenceGrant:v1beta1")

for entry in "${files[@]}"; do
  IFS=':' read -r file kind ver <<< "$entry"
  yaml="$tmpdir/$file.yaml"
  echo "Fetching $base/gateway.networking.k8s.io_${file}.yaml ..."
  # -f: fail on HTTP error status so a 404 page can never be parsed
  curl -sfL "$base/gateway.networking.k8s.io_${file}.yaml" --max-time 60 -o "$yaml"
  ...
  if [ ! -s "$tmpdir/$kind.full.json" ] || ! python3 -c "import json,sys; d=json.load(open('$tmpdir/$kind.full.json')); p=d.get('properties',{}); sys.exit(0 if isinstance(d,dict) and d.get('type')=='object' and 'metadata' in p and 'apiVersion' in p else 1)"; then
    echo "ERROR: version $ver not found (or not a full-object schema) in $file.yaml for tag $GATEWAY_API_VERSION" >&2
    exit 1
  fi
done

python3 - "$tmpdir" "$target" "$GATEWAY_API_VERSION" <<'EOF'
import json, os, sys

tmpdir, target, tag = sys.argv[1], sys.argv[2], sys.argv[3]

# (kind, version) pairs — mirror the gateway entries in src/k8s/kinds.go.
kinds = [("Gateway", "v1"), ("HTTPRoute", "v1"), ("ReferenceGrant", "v1beta1")]
...
for kind, ver in kinds:
    ...
    name = f"io.k8s.api.gateway.networking.k8s.io.{ver}.{kind}"
    defn = {
        "description": f"Gateway API {kind} ({ver}), from the kubernetes-sigs/gateway-api CRD schema ({tag}).",
        "type": "object",
        "properties": props,
    }
...
print(f"swagger/crd/gateway-api.json: {len(defs)} definitions ({', '.join(f'{k} {v}' for k, v in kinds)})")
EOF

sha=$(sha256sum "$target" | cut -d' ' -f1)

echo
echo "Updated $target from kubernetes-sigs/gateway-api@$GATEWAY_API_VERSION (config/crd/standard)"
echo "sha256: $sha"
echo
